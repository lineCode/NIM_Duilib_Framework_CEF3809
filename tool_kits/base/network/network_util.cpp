...
bool GetMacAddressByAdaptersInfo(std::string &mac_address)
{
	...
	if(GetAdaptersInfo(adapter_info, &out_buf_len) == NO_ERROR)
	{
		...
		for(; adapter != NULL; adapter = adapter->Next)
		{
			...
			//过滤虚拟机网卡
			if (strstr(strlwr(adapter->Description), "virtual"))
			{
				continue;
			}

			//过滤不可用ip
			if (!strcmp(adapter->IpAddressList.IpAddress.String, "0.0.0.0"))
			{
				continue;
			}
...
bool GetMacAddress(std::string &mac_address)
{
	...
	bool  success;
	if (true == (success = GetMacAddressByAdaptersInfo(mac_address)))
	{
	...
	else if (true == (success = GetMacAddressByNetBIOS(mac_address)))
	{
