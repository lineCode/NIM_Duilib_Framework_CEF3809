...
#define WM_DPICHANGED       0x02E0
	...
	protected:
		...
		/**
		 * @brief 创建窗口时，若GetSkinFile返回为空则调用本函数加载XML内容
		 * @return 子类可实现并返回窗口皮肤 XML 描述内容
		 */
		virtual std::wstring GetSkinText();
		...
		virtual bool OnMaxBtnClick(EventArgs* param);
		virtual bool OnMinBtnClick(EventArgs* param);
		virtual bool OnCloseBtnClick(EventArgs* param);
		virtual bool OnRestoreBtnClick(EventArgs* param);
	...
	private:
		 ...
		 * @return 始终返回 true
		 */
		void SetMaxBtn(Control* button);
		void SetMinBtn(Control* button);
		void SetRestoreBtn(Control* button);
		void SetCloseBtn(Control* button);

		

	private:
		Control* m_pMaxBtn = nullptr;
		Control* m_pMinBtn = nullptr;
		Control* m_pRestoreBtn = nullptr;
		Control* m_pCloseBtn = nullptr;
	};
}
