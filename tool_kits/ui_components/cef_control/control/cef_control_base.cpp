...
bool CefControlBase::OnDragEnter(CefRefPtr<CefBrowser>browser, CefRefPtr<CefDragData> dragData, CefDragHandler::DragOperationsMask mask)
{
	if (cb_drag_enter_)
		return cb_drag_enter_(browser, dragData, mask);
	return false;
}
...
CefResourceRequestHandler::ReturnValue CefControlBase::OnBeforeResourceLoad(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefRefPtr<CefRequest> request, CefRefPtr<CefRequestCallback> callback)
{
...
CefBrowserSettings CefControlBase::GetBrowserSettings()
{
	if (cb_get_browser_sttings_)
		return cb_get_browser_sttings_();
