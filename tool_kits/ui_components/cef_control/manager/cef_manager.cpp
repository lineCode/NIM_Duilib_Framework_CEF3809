...
bool CefManager::Initialize(const std::wstring& app_data_dir, CefSettings &settings, bool is_enable_offset_render /*= true*/, 
	const RegisterCustomSchemesCallback& register_custom_schemes_callback, const BeforeCommandLineProcessingCallback& before_command_line_processing_callback)
{
#if !defined(SUPPORT_CEF)
	return true;
#endif
	...
	app->AttachRegisterCustomSchemes(register_custom_schemes_callback);
	app->AttachBeforeCommandLineProcessing(before_command_line_processing_callback);
